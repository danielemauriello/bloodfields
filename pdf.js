...
// Builds a PDF (as a Buffer): faction reference pages (description, per-type
// abilities, loyalty bonus) followed by the unit card grid, PER_PAGE cards
// per page. `cards`: [{ name, imagePath }], already in display order.
// `factions`: [{ faction, description, abilities, bonus }], in display order.
function buildRosterPdf(cards, title, subtitle, factions = []) {
